...
function send (data, extension, status) {
    let hdrs = headers, bag;
...
const kv = (function (kv) {
    ...
    return kv
})(await Deno.openKv()), js = extensions.slice(1), tlsKey = resolve((Deno.env.get("PORTFOLIO_TLS_KEY") || "./tls.key")), tlsCrt = resolve((Deno.env.get("PORTFOLIO_TLS_CRT") || "./tls.crt")), host = (function () {
    try {
...
let hostname = toString(host?.hostname, {
    default: false
}), options = {
    port,
    onError: error
...
async function jsImport ({
    ...
    return (data instanceof Response) == true ? data : ["type", "props"].every(function (property) {
        try {
        ...
        try {
            return await import(app).then(async ({
                ...
                return (appData instanceof Response) == true ? appData : ["type", "props"].every(function (property) {
                    try {
                        return property in appData == true
                    } catch (error) {
                        return false
                    }
                }) == true ? send(`<!DOCTYPE html>${renderer(appData)}`, ".html", this?.status) : send(appData, (function () {
                    const name = app.slice(app.lastIndexOf(separator) + 1);
                    return name.slice(name.lastIndexOf(".")).toLowerCase()
                })(), this?.status)
            })
        } catch (err) {
            return send(`<!DOCTYPE html>${renderer(data)}`, ".html", this?.status)
        }
    })() : send(data, this?.extension, this?.status)
};
...
Deno.serve(options, function (request) {
    try {
        const url = new URL(request.url), pathname = url.pathname.slice(url.pathname.lastIndexOf("/") + 1).toLowerCase();
